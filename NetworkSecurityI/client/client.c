...
#include <arpa/inet.h> // int inet_pton
...
//This table stores pre-calculated values for all possible GF(2^8) calculations.
//gf_mul is for mixcolumn and invmixcolumn
//Cause multiply by 01 is itself, so we create a lookup table.
//first two column is the table multiply by 02 and 03 for mixcolumn part
//the last four column is the table multiply by 09, 0b, 0d, 0e for invmixcolumn part
//This table can be get free from https://en.wikipedia.org/wiki/Rijndael_mix_columns
...
void InvMixColumns(uchar state[][4])
{
    ...
    for (i = 0; i<4; i++) {
        ...
        state[0][i] = gf_mul[col[0]][5] ^ gf_mul[col[1]][3] ^ gf_mul[col[2]][4] ^ gf_mul[col[3]][2];
        state[1][i] = gf_mul[col[0]][2] ^ gf_mul[col[1]][5] ^ gf_mul[col[2]][3] ^ gf_mul[col[3]][4];
        state[2][i] = gf_mul[col[0]][4] ^ gf_mul[col[1]][2] ^ gf_mul[col[2]][5] ^ gf_mul[col[3]][3];
        state[3][i] = gf_mul[col[0]][3] ^ gf_mul[col[1]][4] ^ gf_mul[col[2]][2] ^ gf_mul[col[3]][5];
    }
}
...
void aes_decrypt(uchar in[], uchar out[], uint key[])
{
    uchar state[4][4];
    int i,j;
    for (i=0; i<4; i++) {
    ...
    for (i=9; i>0; i--) {
        InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[4*i]);InvMixColumns(state);
    }
 ...
 ** 2. While connected, generated a number by using BBS XB.
 ** 3. Caculate YB = (a^XB) mod p, where p = 1223, a = 5,
 ** 4. Send YB to server and recv YA from server
 ** 5. Caculate K = (YA^XB) mod p by using fastmodular function
 ** 6. While the lenth of K is less than 128 bits, repeat step 2,3,4,5
 ** 7. Using the 128-bits K as the key of AES-128 key
 ** 8. Client ask for file by input the filename
 ** 9. Rec the encryption file of the file
 ** 10. Decrypt the file by using AES-128
 ** 11. Write the plaintext to newfile
 ** 12. Close socket
 ********************/
...
int main()
{
  ...
  if(client_socket_fd < 0)
  {
    ...
    printf("[Client] Create Socket Successful!!!\n");
  
  // bind server socket and client socket addr
  ...
  if(connect(client_socket_fd, (struct sockaddr*)&server_addr, server_addr_length) < 0)
  {
    ...
    printf("[Client] Connect to Server Successful!!!\n");
    ...
    printf("[Client] Start using BBS to generate XB, and use the value of p and a to caculate YB\n");
    printf("[Client] Then Send YB to Server, and receive YA from Server, for creating the common key\n\n");
    printf("Xb\t\tSend YB\t\tRecv YA\t\tCommon Security Key\t\tTransfer Key to binary");
    
    while (i>=0) {
        XB = get_Xab();
        YB = fastmodular(a, XB, p);
        send(client_socket_fd,&YB,sizeof(YB),0);
        recv(client_socket_fd,&YA,sizeof(YA),0);
        K = fastmodular(YA, XB, p);
        printf("%d\t\t%d\t\t%d\t\t%d\t\t\t\t", XB,YB,YA,K);bin(K);
        while (K > 0 && i >= 0  ) {
            if (K % 2 ==0) {
                key[i] = 0;
                K = K/2;
    ...
    printf("\n\n[Client] The Common Secrete Key now equal to 128-bits, this will be using for AES encryption.\n");
    sleep(1);
    ...
    printf("[Client] Final 128-bits Common Security key in binary:\n");
    for (i=0; i<128; i++) {
        printf("%d",key[i]);
    }
    printf("\n[Client] Final 128-bits Common Security Key in Hex:\t");
    for(i = 0; i<sizeof(key1); i++){
  ...
  printf("\n[Client] Please Input the File Name you want to transfer(e.g. 1111.txt): ");
  scanf("%s", file_name);
  ...
  if(send(client_socket_fd, buffer, BUFFER_SIZE, 0) < 0)
  {
    ...
    printf("[Client] Send File Name Successful.\n");
  // open file
  ...
  bzero(buffer, BUFFER_SIZE);
    ...
    uchar cipertext[16];    // the buffer of recv document from server
    uchar plaintext[16];
    uint key_schedule[60],idx;
    KeyExpansion(key1,key_schedule);     // key expansion
    
  int length = 0;
  while((length = recv(client_socket_fd, cipertext, 32, 0)) > 0)
  {
      printf("[Client] Receive data from Server Successful.\n");
      printf("[Client] Cipertext:\t");
      for (idx=0; idx < 16; idx++)
          printf("0x%02x ",cipertext[idx]);
      puts("");
      printf("[Client] Start AES decryption...\n");
      sleep(1);
      aes_decrypt(cipertext,plaintext,key_schedule);
      printf("[Client] Decryption Successful.\n");
      printf("[Client] Plaintext:\t");
      for (idx=0; idx < 16; idx++)
          printf("%c",plaintext[idx]);
      puts("");
    if(fwrite(plaintext, sizeof(char), length, fp) < length)
    {
      printf("File:\t%s Write Failed\n", file_name);
      break;
    }
      printf("[Client] Create File %s, and start write data\n",file_name);
    bzero(cipertext, BUFFER_SIZE);
  }
  
  // close socket
  printf("[Client] Receive File:%s From Server Successful!\n", file_name);
  fclose(fp);
